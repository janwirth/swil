...
pub fn render_instr_where_fn() -> String {
  let subject =
    gex.tuple2(gex.unchecked_ident("left"), gex.unchecked_ident("right"))
  let case_expr =
    gcase.new(subject)
    |> gcase.with_matcher(
      gmatch.tuple2(
    ...
    |> gcase.with_matcher(
      ...
      fn(pair) {
        ...
        let prepared = gex.call2(frag_prep(), sql, gex.list([gex.call1(frag_str(), rv)]))
    ...
    |> gcase.with_matcher(
      ...
      fn(pair) {
        ...
        let prepared = gex.call2(frag_prep(), sql, gex.list([gex.call1(frag_str(), lv)]))
        gex.call1(w_fragment(), prepared)
      },
    )
    |> gcase.with_matcher(
...
pub fn render_bool_expr_where_case_lines() -> String {
  ...
  <> rex(
    gex.call1(
      w_and(),
      gex.list([gex.call1(b, gex.unchecked_ident("left")), gex.call1(b, gex.unchecked_ident("right"))]),
    ),
  )
