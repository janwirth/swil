      ...
      case dec.field_is_calendar_date(f) {
          ...
          case type_is_option_date(f.type_) {
            ...
            False ->
              case type_is_plain_date(f.type_) {
                True ->
                  string.concat([
                    "sqlight.text(api_help.date_to_db_string(",
                    label,
                    "))",
                  ])
                False ->
        ...
        False ->
          case render_type_plain(f, scalar_names) {
            "Float" ->
              string.concat([
                "sqlight.float(api_help.opt_float_for_db(",
                label,
                "))",
              ])
            "Int" ->
              string.concat([
                "sqlight.int(api_help.opt_int_for_db(",
                label,
                "))",
              ])
            "Timestamp" ->
              string.concat([
                "sqlight.int(api_help.opt_timestamp_for_db(",
                label,
                "))",
              ])
            _ ->
              string.concat([
                "sqlight.text(api_help.opt_text_for_db(",
                label,
                "))",
              ])
          }
...
fn patch_set_assignment_string_literal(label: String) -> String {
  let frag = string.concat([migration_sql.quote_ident(label), " = ?"])
    ...
    False ->
      case dec.field_is_calendar_date(f) {
        ...
        False ->
          case render_type_plain(f, scalar_names) {
            "Float" ->
              string.concat(["sqlight.float(", inner_var, ")"])
            "Int" -> string.concat(["sqlight.int(", inner_var, ")"])
            "Timestamp" ->
              string.concat([
            ...
            _ -> string.concat(["sqlight.text(", inner_var, ")"])
          }
      }
  }
}
...
fn patch_sql_prefix_lit(table: String) -> String {
  escape_gleam_string_body(
    string.concat(["update ", migration_sql.quote_ident(table), " set "]),
  )
}
...
fn patch_option_field_block(
  ...
  let inner = string.concat([ref, "_pv"])
...
fn patch_where_sql_suffix_lit(id_cols: List(String)) -> String {
  let where_id =
    id_cols
    |> list.map(fn(c) {
      string.concat([migration_sql.quote_ident(c), " = ?"])
    })
    |> string.join(" and ")
  string.concat([
    " where ",
...
fn patch_identity_plan_case_body(
  ...
  let field_blocks =
    list.map(non_id, fn(f) {
      let ref = string.concat([field_qualifier, f.label])
  ...
  let where_binds =
    list.map(variant.fields, fn(f) {
      string.concat([
        "          ",
        cmd_bind_typed_value(f, string.concat([field_qualifier, f.label])),
        ",",
      ])
    })
    |> string.join("\n")
...
fn delete_binding_lines(variant: IdentityVariantDefinition) -> String {
  let id_lines =
    list.map(variant.fields, fn(f) {
      string.concat(["        ", cmd_bind_typed_value(f, f.label), ","])
    })
    |> string.join("\n")
  string.concat([
    "        sqlight.int(now),\n        sqlight.int(now),\n",
...
fn variant_pattern_labels(fields: List(FieldDefinition)) -> String {
  list.map(fields, fn(f) { string.concat([f.label, ":"]) })
  |> string.join(", ")
}
...
fn op_variant_name(
  ...
  string.concat([op, entity_type, variant.variant_name])
}

fn with_variant_tag_branch(
  case_x: gcase.CaseExpression(gtypes.Dynamic, Int),
  variant_name: String,
  tag: Int,
) -> gcase.CaseExpression(gtypes.Dynamic, Int) {
  gcase.with_pattern(
    case_x,
    gpat.foreign_variant(string.concat([variant_name, "(..)"]), [])
      |> gpat.to_dynamic,
    fn(_) { gexpr.int(tag) },
  )
}

fn variant_tag_case_expression(
  entity_type: String,
  id_def: IdentityTypeDefinition,
) -> gexpr.Expression(Int) {
  let #(case_built, n_after) =
    list.fold(id_def.variants, #(gcase.new(gexpr.raw("cmd")), 0), fn(acc, v) {
      let #(c, n) = acc
      let c =
        c
        |> with_variant_tag_branch(op_variant_name("Upsert", entity_type, v), n)
        |> with_variant_tag_branch(
          op_variant_name("Update", entity_type, v),
          n + 1,
        )
        |> with_variant_tag_branch(op_variant_name("Patch", entity_type, v), n + 2)
        |> with_variant_tag_branch(
  ...
  case_built
  |> with_variant_tag_branch(
    string.concat(["Patch", entity_type, "ById"]),
    n_after,
  )
  |> with_variant_tag_branch(
    string.concat(["Update", entity_type, "ById"]),
    n_after + 1,
  )
...
fn cmd_module_needs_schema(ctx: dec.TypeCtx, def: SchemaDefinition) -> Bool {
  let qualified = fn(f: FieldDefinition) {
    string.contains(
      dec.render_type(f.type_, ctx),
      string.concat([ctx.schema_alias, "."]),
    )
  }
  list.any(def.entities, fn(e) {
...
fn build_command_custom_type(
  ...
  let by_id_variants = [
    gvariant.new(string.concat(["Update", entity_type, "ById"]))
      |> gvariant.with_arguments_dynamic(by_id_args),
    gvariant.new(string.concat(["Patch", entity_type, "ById"]))
      |> gvariant.with_arguments_dynamic(by_id_args),
  ]
...
fn entity_sql_constant_entries(
  ...
  let per_variant =
    list.flat_map(id_def.variants, fn(v) {
      ...
      [
        #(
          string.concat([table, "_upsert_by_", id_snake, "_sql"]),
          api_sql.upsert_sql_exec(table, data_cols, id_cols),
        ),
        #(
          string.concat([table, "_update_by_", id_snake, "_sql"]),
          api_sql.update_by_identity_sql_exec(table, data_cols, id_cols),
        ),
        #(
          string.concat([table, "_delete_by_", id_snake, "_sql"]),
  ...
  list.append(per_variant, [
    #(
      string.concat([table, "_update_by_id_sql"]),
...
fn generate_entity_cmd_piece(
  ...
  let command_type_name = string.concat([entity_type, "Command"])
  ...
  let plan_cases =
    list.map(id_def.variants, fn(v) {
      ...
      let upsert_sql_const =
        string.concat([table, "_upsert_by_", id_snake, "_sql"])
      let update_sql_const =
        string.concat([table, "_update_by_", id_snake, "_sql"])
      let delete_sql_const =
        string.concat([table, "_delete_by_", id_snake, "_sql"])
...
fn entity_cmd_fns_module(
  ...
  let id_def = schema_context.find_identity(def, entity)
  let entity_type = entity.type_name
  let tag_expr = variant_tag_case_expression(entity_type, id_def)
  use _ <- gmod.with_function(
    gleamgen_emit.pub_def(string.concat(["execute_", entity_snake, "_cmds"])),
    gfun.new2(
      gparam.new("conn", gtypes.raw("sqlight.Connection"))
        |> gparam.with_label("conn"),
      gparam.new(
        "commands",
        gtypes.raw(string.concat(["List(", command_type_name, ")"])),
      )
  ...
  use _ <- gmod.with_function(
    gdef.new(string.concat([entity_snake, "_variant_tag"])),
    gfun.new1(
  ...
  use _ <- gmod.with_function(
    gdef.new(string.concat(["plan_", entity_snake])),
    gfun.new2(
      gparam.new("cmd", gtypes.raw(command_type_name))
        |> gparam.with_label("cmd"),
...
fn chain_cmd_module_imports(
  ...
  apply_optional_cmd_imports(
    ...
    fn() {
      ...
      use _ <- gmod.with_import(import_pre(["swil", "runtime", "cmd_runner"]))
      build()
    },
  )
}
