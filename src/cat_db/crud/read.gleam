...
import cat_db/structure.{  type CatField, type CatRow, type FilterableCat, type NumRefOrValue,
...
pub fn read_one(conn: sqlight.Connection, id: Int) -> Result(
  Option(CatRow),
  sqlight.Error,
) {
  use rows <- result.try(
...
fn read_many_filter_where(
  ...
  case arg {
    ...
    filter.FilterArg(f, _) -> crud_filter.bool_expr_where(
      f(crud_filter.filterable_refs()),
    )
  }
}
...
fn read_many_ordered(
  ...
  case order {
    None -> base
    Some(filter.Asc(f)) -> select.order_by_asc(base, crud_sort.cat_field_sql(f))
    Some(filter.Desc(f)) -> select.order_by_desc(
      base,
      crud_sort.cat_field_sql(f),
    )
  }
}
