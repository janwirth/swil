...
pub fn upsert_one(
  ...
  case cat {
    CatWithName(name, age) -> {
      use _ <- result.try(cake_sql_exec.run_write_query(
        cake_insert.to_query(cake_insert.on_columns_conflict_update(
          cake_insert.source_values(
            cake_insert.columns(cake_insert.table(cake_insert.new(), "cats"), [
              "name",
              "age",
              "created_at",
              "updated_at",
            ...
            [
              cake_insert.row([
                cake_insert.string(name),
                case age {
                  Some(v) -> cake_insert.int(v)
                  None -> cake_insert.null()
                },
                cake_insert.int(stamp_sec),
                cake_insert.int(stamp_sec),
                cake_insert.null(),
              ]),
            ],
          ...
          cake_update.set(
            cake_update.set(
              ...
              cake_update.set_expression("updated_at", "excluded.updated_at"),
            ),
      ...
      result.map(
        cake_sql_exec.run_read_query(
          select.to_query(select.where(
            select.select_cols(select.from_table(select.new(), "cats"), [
              "id",
              "created_at",
              "updated_at",
              "deleted_at",
...
pub fn upsert_many(
  ...
  list.try_map(rows, fn(c: CatForUpsert) -> Result(CatRow, sqlight.Error) {
    upsert_one(conn, c)
  })
}
