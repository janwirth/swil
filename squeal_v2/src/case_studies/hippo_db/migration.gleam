...
fn first_surplus_column_hippo(rows: List(TableInfoRow), wanted: List(HippoCol)) -> Option(String) {
  case
...
fn first_missing_column_hippo(rows: List(TableInfoRow), wanted: List(HippoCol)) -> Option(HippoCol) {
  case
...
fn reconcile_hippo_columns_loop(conn: sqlight.Connection, iter: Int) -> Result(Nil, sqlight.Error) {
  case iter > 64 {
  True ->
  ...
  sqlite_pragma_assert.index_info_tsv(conn, "hippo_by_name_date_of_birth")
{
  Ok(list_tsv), Ok(info_tsv) ->
    case list_tsv == expected_hippo_index_list && info_tsv == expected_hippo_index_info {
      True -> Ok(Nil)
      False -> {
...
fn first_surplus_column_human(rows: List(TableInfoRow), wanted: List(HumanCol)) -> Option(String) {
  case
  list.find(rows, fn(r) { !list.any(wanted, fn(w) { w.name == r.name }) })
{
  Ok(r) -> Some(r.name)
...
fn first_missing_column_human(rows: List(TableInfoRow), wanted: List(HumanCol)) -> Option(HumanCol) {
  case
  list.find(wanted, fn(w) { !list.any(rows, fn(r) { r.name == w.name }) })
{
  Ok(w) -> Some(w)
...
fn reconcile_human_columns_loop(conn: sqlight.Connection, iter: Int) -> Result(Nil, sqlight.Error) {
  case iter > 64 {
  True ->
    panic as "case_studies/hippo_db/migration: column reconcile did not converge"
  False -> {
