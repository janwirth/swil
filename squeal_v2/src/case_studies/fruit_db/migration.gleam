...
fn first_surplus_column(rows: List(TableInfoRow), wanted: List(FruitCol)) -> Option(String) {
  case
...
fn first_missing_column(rows: List(TableInfoRow), wanted: List(FruitCol)) -> Option(FruitCol) {
  case
...
fn reconcile_fruit_columns_loop(conn: sqlight.Connection, iter: Int) -> Result(Nil, sqlight.Error) {
  case iter > 64 {
  True ->
