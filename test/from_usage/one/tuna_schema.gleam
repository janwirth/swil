...
pub type ImportedTrack {
  ImportedTrack(
    ...
    // source specific id (user can have multiple of type)
...
pub type Tag {
  Tag(label: option.Option(String), identities: TagIdentities)
}
...
pub fn query_track_by_source_root(
  ...
  |> dsl.shape(track)
  |> dsl.filter_bool(
    dsl.exclude_if_missing(track.from_source_root) == source_root,
  )
