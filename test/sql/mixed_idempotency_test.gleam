...
pub fn imported_track_evolution_v1_pragma_codegen_matches_disk_test() {
  ...
  let assert Ok(expected) =
    simplifile.read(
      "src/case_studies/imported_track_evolution_v1_db/migration.gleam",
    )
  let assert Ok(def) = schema_parser.parse_module(src)
  let assert Ok(gleam) =
    migration.generate_pragma_migration_module(
...
pub fn imported_track_evolution_v2_pragma_codegen_matches_disk_test() {
  let assert Ok(src) =
    simplifile.read("src/case_studies/imported_track_evolution_v2_schema.gleam")
  let assert Ok(expected) =
    simplifile.read(
      "src/case_studies/imported_track_evolution_v2_db/migration.gleam",
    )
  let assert Ok(def) = schema_parser.parse_module(src)
  let assert Ok(gleam) =
    migration.generate_pragma_migration_module(
