...
pub fn dogs_migration_idempotent_three_times_test() {
  ...
  let assert Ok(row) =
    dogs.dogs(conn).upsert_one(dogs.dog_with_name_is_neutered("Rex", True, Some(5)))
  let assert DogRow(
...
pub fn dogs_filter_by_neutered_test() {
  ...
  let assert Ok(_row) =
    dogs.dogs(conn).upsert_one(
      dogs.dog_with_name_is_neutered("Rex", True, Some(5)),
    )
  let assert Ok(_row) =
    dogs.dogs(conn).upsert_one(
      dogs.dog_with_name_is_neutered("Bolt", False, Some(3)),
    )
  let assert Ok(_row) =
    dogs.dogs(conn).upsert_one(
      dogs.dog_with_name_is_neutered("Luna", True, Some(2)),
    )
...
pub fn dogs_upsert_composite_identity_test() {
  ...
  let assert Ok(updated_same_identity) =
    dogs.dogs(conn).upsert_one(
      dogs.dog_with_name_is_neutered("Rex", True, Some(6)),
    )
  // same name, different neutered flag -> distinct row
  let assert Ok(second_identity) =
    dogs.dogs(conn).upsert_one(
      dogs.dog_with_name_is_neutered("Rex", False, Some(4)),
    )
  ...
  let assert Ok(all_rows) = dogs.dogs(conn).read_many(dogs_crud.filter_arg(None, None))
  let assert [row1, row2] = list.sort(all_rows, by: fn(a, b) { int.compare(a.id, b.id) })
  let assert DogRow(
