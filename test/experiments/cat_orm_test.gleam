...
import cat_example.{
  cat_age_eq, cat_name_excludes, cat_older_than,
  cat_older_than_and_name_excludes,
}
...
pub fn cats_typed_schema_test() {
  ...
  let whiskers = cats.cat_with_name("Whiskers", Some(3))
  let luna = cats.cat_with_name("Luna", Some(10))
  ...
  let assert Ok(rows) =
    cats.cats(conn).read_many(filter.FilterArg(
      filter: cat_older_than(6),
      sort: None,
    ))
  let assert [nubi_row, luna_row] =
  ...
  let assert Ok(rows) =
    cats.cats(conn).read_many(filter.FilterArg(
      filter: cat_name_excludes("ubi"),
      sort: None,
    ))
  let assert [whiskers_row, luna_row2] =
    list.sort(rows, by: fn(a, b) { int.compare(a.id, b.id) })
  let assert CatRow(
    value: Cat(name: Some("Whiskers"), age: Some(3)),
    id: 2,
  ...
  let assert [composite_nubi, composite_luna] =
    list.sort(rows, by: fn(a, b) { int.compare(a.id, b.id) })
  let assert CatRow(
    value: Cat(name: Some("Nubi"), age: Some(7)),
    id: 1,
  ...
  let assert Ok(sorted_by_age_desc) =
    cats.cats(conn).read_many(
      filter.NoFilter(sort: Some(filter.Desc(AgeField))),
    )
  let assert [
    CatRow(
      value: Cat(name: Some("Luna"), age: Some(10)),
...
pub fn cats_update_one_test() {
  ...
  let assert Ok(Some(updated)) =
    cats.cats(conn).update_one(
      inserted.id,
      Cat(name: Some("Nubi"), age: Some(9)),
    )
...
pub fn cats_update_many_test() {
  ...
  let assert Ok(r1) =
    cats.cats(conn).upsert_one(cats.cat_with_name("Nubi", Some(7)))
  let assert Ok(r2) =
    cats.cats(conn).upsert_one(cats.cat_with_name("Luna", Some(10)))
  ...
  let assert Ok([Some(first), Some(second), None]) =
    cats.cats(conn).update_many(updates)
