...
pub fn migrate_idempotent(conn: sqlight.Connection) -> Result(Nil, sqlight.Error) {
  ...
  use _ <- result.try(migration_help.ensure_column(conn, "age", "alter table cats add column age int;"))
  migration_help.ensure_column(conn, "gender", "alter table cats add column gender text;")
}
